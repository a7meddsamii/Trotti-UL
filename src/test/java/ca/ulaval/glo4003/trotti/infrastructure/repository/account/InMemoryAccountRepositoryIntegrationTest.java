...
class InMemoryAccountRepositoryIntegrationTest {
	...
	void setup() {
		...
		accountRepository = new InMemoryAccountRepository(userInMemoryDatabase);
	}
	...
	void givenIdulOfNonExistingAccount_whenFindingByIdul_thenReturnsEmpty() {
		Optional<Account> retrievedAccount = accountRepository.findByIdul(IDUL_OF_NON_EXISTING_ACCOUNT);
