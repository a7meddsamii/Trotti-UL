...
class DischargingStrategyTest {
	private static final LocalDateTime CURRENT_TIME = LocalDateTime.of(2024, 1, 1, 12, 0);
	private static final BatteryLevel UPDATED_BATTERY_LEVEL = BatteryLevel.from(BigDecimal.ZERO);
	private static final BatteryLevel INITIAL_BATTERY_LEVEL = BatteryLevel.from(BigDecimal.TEN);
	private static final long ELAPSED_MINUTES = 50;
	private final DischargingStrategy strategy = DischargingStrategy.INSTANCE;

    @Test
    void givenNoTimeElapsed_whenComputeLevel_thenBatteryUnchanged() {
		BatteryLevel result = strategy.computeLevel(CURRENT_TIME, CURRENT_TIME, INITIAL_BATTERY_LEVEL);
