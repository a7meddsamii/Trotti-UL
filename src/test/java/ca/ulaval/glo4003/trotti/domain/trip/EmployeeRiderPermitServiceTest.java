...
class EmployeeRiderPermitServiceTest {
	...
	private static final Session A_SESSION = new Session(Semester.FALL, LocalDate.of(2026, 1, 1), LocalDate.of(2026, 6, 30));
	...
	void givenEmployeeIdulAndValidCurrentDate_whenHandleEmployeeRidePermit_thenAddPermitToEmployee() {
		...
		Mockito.when(sessionRegistry.getSession(Mockito.any(LocalDate.class))).thenReturn(java.util.Optional.of(A_SESSION));
	...
	void givenEmployeeIdulAndInvalidCurrentDate_whenHandleEmployeeRidePermit_thenDoNotAddPermitToEmployee() {
		Mockito.when(employeeRegistry.isEmployee(traveler.getIdul())).thenReturn(true);
		Mockito.when(sessionRegistry.getSession(Mockito.any(LocalDate.class))).thenReturn(java.util.Optional.empty());
