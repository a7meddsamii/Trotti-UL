...
class BatteryTest {
	...
	private static final BatteryLevel BELOW_MINIMUM_OPERATIONAL_LEVEL = BatteryLevel.from(BigDecimal.valueOf(10));
	private static final BatteryLevel ABOVE_MINIMUM_OPERATIONAL_LEVEL = BatteryLevel.from(BigDecimal.valueOf(20));
	...
	void givenUpdateTimeBeforeLastUpdate_whenChangeBatteryState_thenThrowsException() {
		...
		Executable changeState = () -> battery.changeBatteryState(BatteryState.DISCHARGING, updateTime);
	...
	void whenHasEnoughCharge_thenReturnsTrue() {
		battery = new Battery(ABOVE_MINIMUM_OPERATIONAL_LEVEL, LAST_BATTERY_UPDATE, BatteryState.CHARGING);
	...
	void whenDoesNotHaveEnoughCharge_thenReturnsFalse() {
		battery = new Battery(BELOW_MINIMUM_OPERATIONAL_LEVEL, LAST_BATTERY_UPDATE, BatteryState.CHARGING);
