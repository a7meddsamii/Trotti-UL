...
class BatteryLevelStateTest {
	...
	void givenDischargingState_whenComputeLevel_thenUsesDischargeStrategy() {
		BatteryLevel expectedResult = DischargingStrategy.INSTANCE.computeLevel(LAST_BATTERY_UPDATE, CURRENT_TIME, A_BATTERY_LEVEL);
		
		BatteryLevel result = BatteryState.DISCHARGING.computeLevel(A_BATTERY_LEVEL, LAST_BATTERY_UPDATE, CURRENT_TIME);
	...
	void givenChargingState_whenComputeLevel_thenUsesChargeStrategy() {
		BatteryLevel expectedResult = ChargingStrategy.INSTANCE.computeLevel(LAST_BATTERY_UPDATE, CURRENT_TIME, A_BATTERY_LEVEL);
		
		BatteryLevel result = BatteryState.CHARGING.computeLevel(A_BATTERY_LEVEL, LAST_BATTERY_UPDATE, CURRENT_TIME);
