...
class TravelerControllerTest {
    ...
    void setup() {
        ...
        resource = new TravelerController(ridePermitService, authenticationService);
    ...
    void givenValidToken_whenGetRidePermits_thenReturns200() {
        ...
        Mockito.when(ridePermitService.getRidePermits(idul)).thenReturn(permits);
        ...
        Mockito.verify(ridePermitService).getRidePermits(idul);
    }

    @Test
    void givenValidTokenAndNoPermits_whenGetRidePermits_thenReturns200AndEmptyList() {
        stubValidAuthentication();
        Mockito.when(ridePermitService.getRidePermits(idul)).thenReturn(List.of());
    ...
    void givenInvalidToken_whenGetRidePermits_thenThrowException() {
        ...
        Executable action = () -> resource.getRidePermits(AUTH_HEADER);
    ...
    void givenRidePermitServiceException_whenGetPermits_thenThrowException() {
        stubValidAuthentication();
        Mockito.when(ridePermitService.getRidePermits(idul))
