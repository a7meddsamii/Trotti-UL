...
class FleetTest {
	...
	void givenMismatchedSlotsAndScooters_whenDepositScooters_thenThrowsInvalidTransferException() {
		List<SlotNumber> slots = List.of(SLOT_1);
		List<ScooterId> displacedScooterIds = List.of(scooter.getScooterId(), secondScooter.getScooterId());
		...
		Executable action = () -> fleet.depositScooters(A_LOCATION, slots, displacedScooterIds, A_TIME);
