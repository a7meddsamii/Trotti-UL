...
class FleetDataFactoryTest {
    ...
    private static final String BUILDING = "VACHON";
    private static final String SPOT_NAME = "Main Entrance";
    ...
    void givenSingleStationData_whenRun_thenCreatesStationWithCorrectLocation() {
        StationDataRecord record = new StationDataRecord(BUILDING, SPOT_NAME, CAPACITY);
        when(stationFactory.create(any(Location.class), eq(CAPACITY))).thenReturn(mockStation);
        ...
        verify(stationFactory).create(locationCaptor.capture(), eq(CAPACITY));

        Location capturedLocation = locationCaptor.getValue();
        assertEquals(BUILDING, capturedLocation.getBuilding());
        assertEquals(SPOT_NAME, capturedLocation.getSpotName());
    }
