...
class TransactionMapperTest {
    ...
    void givenTransaction_whenToDto_thenReturnTransactionDto() {
        CreditCard paymentMethod = new CreditCardFixture().build();
        Transaction transaction =
                new Transaction(TransactionStatus.SUCCESS, Money.zeroCad(), "Test Transaction");

        TransactionDto transactionDto = transactionMapper.toDto(transaction, paymentMethod.getCardNumber());
        ...
        Assertions.assertEquals(paymentMethod.getCardNumber(), transactionDto.cardNumber());
    }
}
