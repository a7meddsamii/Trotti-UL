...
class OrderTest {
	...
	private static final MaximumDailyTravelTime FIRST_MAXIMUM_TRAVEL_TIME = MaximumDailyTravelTime.from(Duration.ofHours(2));
	private static final MaximumDailyTravelTime SECOND_MAXIMUM_TRAVEL_TIME = MaximumDailyTravelTime.from(Duration.ofHours(5));
	...
	void setUp() {
		...
		firstItem = new OrderItem(ItemId.randomId(), FIRST_MAXIMUM_TRAVEL_TIME, session, BillingFrequency.MONTHLY);
		secondItem = new OrderItem(ItemId.randomId(), SECOND_MAXIMUM_TRAVEL_TIME, session, BillingFrequency.PER_TRIP);
		duplicateOfFirst = new OrderItem(firstItem.getItemId(), FIRST_MAXIMUM_TRAVEL_TIME, session, BillingFrequency.MONTHLY);
