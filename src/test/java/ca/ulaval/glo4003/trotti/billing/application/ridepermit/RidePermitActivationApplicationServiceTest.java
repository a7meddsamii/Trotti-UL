...
public class RidePermitActivationApplicationServiceTest {
	...
	void givenCurrentSession_whenActivateRidePermit_thenOrchestratesProperly() {
		...
		Mockito.when(ridePermitRepository.findAllByDate(Mockito.any(LocalDate.class))).thenReturn(ridePermitList);
		Mockito.when(ridePermitActivationFilter.getActivatedRidePermits(ridePermitList)).thenReturn(ridePermitList);
		
		
		ridePermitActivationApplicationService.activateRidePermit();
		
		Mockito.verify(ridePermitRepository, Mockito.times(1)).findAllByDate(Mockito.any(LocalDate.class));
		Mockito.verify(ridePermitActivationFilter, Mockito.times(1)).getActivatedRidePermits(Mockito.anyList());
