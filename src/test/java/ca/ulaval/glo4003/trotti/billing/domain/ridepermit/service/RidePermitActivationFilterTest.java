...
class RidePermitActivationFilterTest {
	...
	void whenGetActivatedRidePermits_thenFilterPermitsBasedOnCurrentSession() {
		...
		List<RidePermit> result = filter.getActivatedRidePermits(List.of(activablePermit, nonActivablePermit));
	...
	void givenNoCurrentSession_whenGettingActivatedRidePermits_thenThrowException() {
		...
		Executable executable = () -> filter.getActivatedRidePermits(List.of(permit));
	...
	void whenGetDeactivatedRidePermits_thenFilterPermitsBasedOnCurrentSession() {
		...
		List<RidePermit> result = filter.getDeactivatedRidePermits(List.of(deactivablePermit, nonDeactivablePermit));
	...
	void givenNoCurrentSession_whenGettingDeactivatedRidePermits_thenThrowException() {
		...
		Executable executable = () -> filter.getDeactivatedRidePermits(List.of(permit));
	...
	void whenGetPreviousSession_thenReturnPreviousSessionFromSchoolSessionProvider() {
		...
		when(schoolSessionProvider.getPreviousSession(CURRENT_DATE)).thenReturn(Optional.of(previousSession));
