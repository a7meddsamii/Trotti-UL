...
class RidePermitActivationApplicationServiceIntegrationTest {
    ...
    private static final Session CURRENT_SESSION = new Session(Semester.FALL, LocalDate.of(2025, 9, 1), LocalDate.of(2025, 12, 20));
    private static final Session PREVIOUS_SESSION = new Session(Semester.SUMMER, LocalDate.of(2025, 5, 1), LocalDate.of(2025, 8, 31));
    ...
    void givenMultipleInactiveRidePermits_whenActivateRidePermit_thenOnlyCurrentSessionPermitsActivated() {
        ...
        Optional<RidePermit> currentPermit = ridePermitRepository.findById(currentSessionPermit.getId());
        Optional<RidePermit> otherPermit = ridePermitRepository.findById(otherSessionPermit.getId());
    ...
    private void setupMocks() {
        Mockito.when(clock.instant()).thenReturn(CURRENT_DATE.atStartOfDay().atZone(Clock.systemDefaultZone().getZone()).toInstant());
        Mockito.when(clock.getZone()).thenReturn(Clock.systemDefaultZone().getZone());
        Mockito.when(schoolSessionProvider.getSession(CURRENT_DATE)).thenReturn(Optional.of(CURRENT_SESSION));
        Mockito.when(schoolSessionProvider.getPreviousSession(CURRENT_DATE)).thenReturn(Optional.of(PREVIOUS_SESSION));
    }

    private RidePermit createInactiveRidePermit(Session session) {
        return new RidePermit(
                RidePermitId.randomId(),
                RIDER_ID,
                session,
                Duration.ofHours(2)
        );
    ...
    private RidePermit createActiveRidePermit(Session session) {
        RidePermit permit = new RidePermit(
                RidePermitId.randomId(),
                RIDER_ID,
                session,
                Duration.ofHours(2)
