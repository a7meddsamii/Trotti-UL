...
class DailyBillingUsageTest {
    ...
    void givenMatchingDate_isOnDateReturnsTrue() {
        DailyBillingUsage usage = new DailyBillingUsage(THIRTY_MINUTES, TODAY, ZERO_DURATION, ZERO_CAD, false);
        assertTrue(usage.isOnDate(TODAY));
    }

    @Test
    void givenDifferentDate_isOnDateReturnsFalse() {
        DailyBillingUsage usage = new DailyBillingUsage(THIRTY_MINUTES, TODAY.minusDays(1), ZERO_DURATION, ZERO_CAD, false);
