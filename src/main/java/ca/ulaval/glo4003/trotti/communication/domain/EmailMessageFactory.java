...
public class EmailMessageFactory {
    ...
    public EmailMessage createUnlockCodeMessage(Email recipientEmail, String name, String unlockCode) {
        String body = "Hello " + name + ", \n"
                + "Here is your code to unlock your scooter : \n" + unlockCode
                + "\n" + "Have a safe ride!\n" + END_TEXT;

        return EmailMessage.builder().withSubject("Unlock Code for your trip")
                .withBody(body)
                .withRecipient(recipientEmail).build();
    }

    public EmailMessage createMaintenanceMessage(Email recipientEmail, String location, String message) {
    ...
    public EmailMessage createRidePermitActivationMessage(Email email, RidePermitSnapshot ridePermitSnapshot) {
        String body = "Your ride permit with ID: " + ridePermitSnapshot.ridePermitId() + " has been activated.\n\n"
                + "Valid for the session: " + ridePermitSnapshot.session() + "\nEffective period: "
                + "\n\t- From: " + ridePermitSnapshot.sessionStartDate() + "\n\t- To: "
                + ridePermitSnapshot.sessionExpirationDate() + ".\n\nEnjoy your rides!"
                + END_TEXT;

        return EmailMessage.builder().withSubject("Ride Permit Activation")
                .withBody(body)
                .withRecipient(email)
                .build();
    }
