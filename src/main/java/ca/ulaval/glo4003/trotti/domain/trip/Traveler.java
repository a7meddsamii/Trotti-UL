...
public class Traveler {
    ...
    public void updateActiveRidePermits(List<RidePermit> ridePermits) {
        List<RidePermit> activeRidePermits = ridePermits.stream().filter(ridePermit -> ridePermit.isActiveFor(LocalDate.now())).toList();
        List<RidePermit> newlyActiveRidePermits = ridePermits.stream().filter(ridePermit -> !this.activeRidePermits.contains(ridePermit)).toList();
    ...
    private void notify(List<RidePermit> ridePermits) {
        for (RidePermit ridePermit : ridePermits) {
            EmailMessage emailMessage = EmailMessage.builder()
                    .withRecipient(this.email)
                    .withSubject("Newly activated pass")
                    .withBody( ridePermit + "has been activated.")
                    .build();
