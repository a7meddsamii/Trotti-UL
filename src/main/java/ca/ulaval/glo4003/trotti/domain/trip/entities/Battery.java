...
public class Battery {
	private static final BatteryLevel MINIMUM_OPERATIONAL_LEVEL = BatteryLevel.from(BigDecimal.valueOf(15));
	private BatteryLevel batteryLevel;
	private BatteryState currentBatteryState;
	private LocalDateTime lastBatteryUpdate;
	
	
	public Battery(BatteryLevel batteryLevel, LocalDateTime lastBatteryUpdate, BatteryState currentBatteryState) {
	...
	public void changeBatteryState(BatteryState newState, LocalDateTime dateTimeOfChange) {
		...
		this.batteryLevel = this.currentBatteryState.computeLevel(
				lastBatteryUpdate,
