...
public class JwtSessionTokenProviderAdapter implements SessionTokenProvider {

    private final Duration expirationDuration;
    private final Clock clock;
    private final SecretKey secretKey;

    public JwtSessionTokenProviderAdapter(
    ...
    public SessionToken generateToken(Idul idul, Role role, Set<Permission> permissions) {
        ...
        String tokenValue = Jwts.builder().subject(idul.toString()).claim("role", role.toString())
    ...
    public AuthenticatedIdentity deserialize(SessionToken token) {
        try {
            ...
            Set<Permission> permissions = permsRaw.stream().filter(String.class::isInstance)
                    .map(String.class::cast).map(Permission::valueOf).collect(Collectors.toSet());
            ...
            throw new AuthenticationException(
                    "Something went wrong during the session authentication");
        }
    }
}
