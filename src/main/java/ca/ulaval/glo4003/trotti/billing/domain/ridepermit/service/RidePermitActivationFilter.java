...
public class RidePermitActivationFilter {
	...
	public List<RidePermit> getDeactivatedRidePermits(List<RidePermit> ridePermits) {
		Session currentSchoolSession = getCurrentSession();
		return ridePermits.stream()
				.filter(ridePermit -> ridePermit.deactivate(currentSchoolSession))
				.toList();
	}
	...
	public Session getPreviousSession() {
		return schoolSessionProvider.getPreviousSession(getCurrentSessionDate())
				.orElseThrow(() -> new NotFoundException("Could not find a previous school session"));
	}
	
	private Session getCurrentSession() {
		LocalDate now = getCurrentSessionDate();
		return schoolSessionProvider.getSession(now).orElseThrow(()->new NotFoundException("Could not find an active school session"));
	}
}
