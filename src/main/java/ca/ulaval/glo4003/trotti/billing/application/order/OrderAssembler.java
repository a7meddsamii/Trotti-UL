...
public class OrderAssembler {
    public OrderDto assemble(Order order) {
        List<RidePermitItemDto> ridePermitItemDtos =
                order.getItems().stream().map(this::assemble).toList();
        return new OrderDto(order.getOrderId(), order.getBuyerId(), ridePermitItemDtos,
                order.getTotalCost(), order.getStatus());
    }

    public List<RidePermitItemSnapshot> assembleRidePermitItemSnapshots(Order order) {
        return order.getItems().stream()
                .map(item -> new RidePermitItemSnapshot(item.getSession(),
                        item.getMaximumTravelingTime().getValue(), item.getBillingFrequency()))
                .toList();

    }

    public RidePermitItemDto assemble(RidePermitItem ridePermitItem) {
        return new RidePermitItemDto(ridePermitItem.getItemId(),
                ridePermitItem.getMaximumTravelingTime(), ridePermitItem.getSession(),
                ridePermitItem.getBillingFrequency(), ridePermitItem.getCost());
    }
}
