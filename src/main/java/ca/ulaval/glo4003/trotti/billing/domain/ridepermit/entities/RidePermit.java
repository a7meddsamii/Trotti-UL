...
public class RidePermit {
    ...
    public RidePermit(
        ...
        this(id, riderId, session, maximumTravelingTimePerDay, new HashMap<>(), RidePermitState.INACTIVE);
    }
    ...
    public RidePermit(
            ...
            Session session,
            Duration maximumTravelingTimePerDay,
            Map<LocalDate, DailyBillingUsage> dailyBillingUsages, RidePermitState permitState) {
        this.id = id;
    ...
    public void addDailyTravelTime(Idul riderId, LocalDateTime startDateTime, Duration travelTime) {
        if (!this.riderId.equals(riderId)) {
			throw new InvalidRidePermitOperation("Rider " + riderId + " does not own this ride permit");
		}
