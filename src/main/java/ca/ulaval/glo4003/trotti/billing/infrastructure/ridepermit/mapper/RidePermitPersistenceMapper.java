...
public class RidePermitPersistenceMapper {
    public RidePermitPersistenceDto toDto(RidePermit ridePermit) {
        ...
        return new RidePermitPersistenceDto(ridePermit.getId(), ridePermit.getRiderId(),
                ridePermit.getSession(), ridePermit.getMaximumTravelingTimePerDay(),
                ridePermit.getPermitState(), usageDtos);
    }
    ...
    public RidePermit toDomain(RidePermitPersistenceDto dto) {
        ...
        return new RidePermit(dto.id(), dto.riderId(), dto.session(),
                dto.maximumTravelingTimePerDay(), dailyUsages, dto.permitState());
    }

    private DailyBillingUsagePersistenceDto toUsageDto(DailyBillingUsage usage) {
        return new DailyBillingUsagePersistenceDto(usage.getMaximumTravelingTimePerDay(),
                usage.getDate(), usage.getTravelingTime(), usage.getBalance(),
                usage.isMaximumTravelingTimeExceeded());
    }

    private DailyBillingUsage toUsageDomain(DailyBillingUsagePersistenceDto dto) {
        return new DailyBillingUsage(dto.maximumTravelingTimePerDay(), dto.date(),
                dto.travelingTime(), dto.balance(), dto.maximumTravelingTimeExceeded());
    }
}
