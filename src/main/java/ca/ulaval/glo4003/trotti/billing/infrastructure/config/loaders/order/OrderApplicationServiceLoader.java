...
public class OrderApplicationServiceLoader extends Bootstrapper {
	...
	private void loadOrderApplicationService() {
		...
		PaymentMethodFactory paymentMethodFactory = this.resourceLocator.resolve(PaymentMethodFactory.class);
		...
		OrderApplicationService orderApplicationService = new OrderApplicationService(
				orderRepository, orderAssembler, orderItemFactory, paymentMethodFactory, paymentGateway, eventBus
		);
