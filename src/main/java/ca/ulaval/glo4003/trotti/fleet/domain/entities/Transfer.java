...
public class Transfer {
    ...
    public List<Scooter> unload(Idul technicianId, int numberOfScooters) {
		...
		List<Scooter> scootersToDeposit = scootersToMove.subList(0, numberOfScooters);
		scootersToMove.removeAll(scootersToDeposit);
	...
	private void validateUnloadOperation(Idul technicianId, int numberOfScooters) {
		if (!this.technicianId.equals(technicianId)) {
			throw new InvalidTransferException("Technician " + technicianId + " is not assigned to this transfer");
		}
		
		if (numberOfScooters <= 0 || numberOfScooters > scootersToMove.size()) {
			throw new InvalidTransferException("Invalid number of scooters to unload: " + numberOfScooters);
		}
