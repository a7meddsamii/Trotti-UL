...
public class Fleet {
	...
	public Fleet(Map<Location, Station> stations, Map<ScooterId, Scooter> rentedScooters, Map<Idul, Transfer> ongoingTransfers) {
		this.stations = stations;
		this.rentedScooters = rentedScooters;
		this.ongoingTransfers = ongoingTransfers;
	}
	
	public ScooterId rentScooter(Location location, SlotNumber slotNumber, LocalDateTime rentStartTime) {
	...
	public void returnScooter(ScooterId scooterId, Location location, SlotNumber slotNumber, LocalDateTime returnTime) {
		Station station = stations.get(location);
	...
	public void startTransfer(Idul technicianId, Location location, List<SlotNumber> slotNumbers) {
		if (ongoingTransfers.containsKey(technicianId)) {
			throw new InvalidTransferException("Technician " + technicianId + " already has an ongoing transfer");
		}
	...
	public void unloadTransfer(Idul technicianId, Location location, List<SlotNumber> slotNumbers, LocalDateTime dockTime) {
		Transfer transfer = ongoingTransfers.get(technicianId);
		
		if (transfer == null) {
			throw new InvalidTransferException("Technician " + technicianId + " does not have an ongoing transfer");
		}
