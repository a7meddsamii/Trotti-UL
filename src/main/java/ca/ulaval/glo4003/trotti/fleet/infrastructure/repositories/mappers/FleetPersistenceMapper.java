...
public class FleetPersistenceMapper {
    ...
    public FleetRecord toRecord(Fleet fleet) {
        Set<StationRecord> stationRecords = fleet.getStations().values().stream()
                .map(this::toStationRecord).collect(Collectors.toSet());

        Set<ScooterRecord> displacedScooterRecords = fleet.getDisplacedScooters().values().stream()
                .map(this::toScooterRecord).collect(Collectors.toSet());
    ...
    public Fleet toDomain(FleetRecord fleetRecord) {
        Map<Location, Station> stations =
                fleetRecord.stationsRecords().stream().map(this::toStationDomain)
                        .collect(Collectors.toMap(Station::getLocation, station -> station));

        Map<ScooterId, Scooter> displacedScooters =
                fleetRecord.displacedScootersRecords().stream().map(this::toScooterDomain)
    ...
    private ScooterRecord toScooterRecord(Scooter scooter) {
        return new ScooterRecord(scooter.getScooterId(), toBatteryRecord(scooter.getBattery()),
                scooter.getLocation());
    }

    private Scooter toScooterDomain(ScooterRecord scooterRecord) {
        return new Scooter(scooterRecord.id(), toBatteryDomain(scooterRecord.batteryRecord()),
                scooterRecord.location());
    }

    private StationRecord toStationRecord(Station station) {
        Map<SlotNumber, ScooterRecord> dockingAreaRecord =
                toDockingAreaRecord(station.getDockingArea());

        return new StationRecord(station.getLocation(), dockingAreaRecord,
    ...
    private DockingArea toDockingAreaDomain(Map<SlotNumber, ScooterRecord> dockingAreaRecord) {
        ...
        dockingAreaRecord.forEach((slotNumber, scooterRecord) -> {
            if (scooterRecord != null) {
                scooterSlots.put(slotNumber,
                        new ScooterSlot(slotNumber, toScooterDomain(scooterRecord)));
            } else {
