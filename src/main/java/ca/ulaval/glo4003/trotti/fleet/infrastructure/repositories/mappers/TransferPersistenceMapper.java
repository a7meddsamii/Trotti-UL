...
public class TransferPersistenceMapper {
	...
	public Transfer toDomain(TransferRecord transferRecord) {
		Map<ScooterId, Boolean> scootersTransferCompletedState = new HashMap<>(transferRecord.scootersTransferCompletedState());
		
		return new Transfer(
				transferRecord.transferId(),
				transferRecord.technicianId(),
				scootersTransferCompletedState
		);
	}
	
	public TransferRecord toRecord(Transfer transfer) {
		Map<ScooterId, Boolean> scootersTransferCompletedState = new HashMap<>(transfer.getScootersTransferCompletedState());
		
		return new TransferRecord(
				transfer.getTransferId(),
				transfer.getTechnicianId(),
				scootersTransferCompletedState
		);
	}
}
