...
public class JsonSessionGateway implements SchoolSessionGateway {
	...
	public JsonSessionGateway(Path resourcePath, SessionMapper sessionMapper, ObjectMapper jsonProvider) {
	...
	public Optional<Session> getSession(LocalDate date) {
		Set<Session> sessions = readSessions().stream().map(sessionMapper::toDomain).collect(Collectors.toSet());
	...
	private Set<SessionRecord> readSessions() {
		try (InputStream input = Files.newInputStream(this.resourcePath)) {
			Set<SessionRecord> sessionRecords = this.jsonMapper.readValue(input, new TypeReference<>() {});
	...
	private Set<SessionRecord> adjustSessionsEndDate(Set<SessionRecord> sessionRecords) {
		List<SessionRecord> sortedSessions = sessionRecords.stream()
				.sorted(Comparator.comparing(SessionRecord::startDate))
				.toList();
