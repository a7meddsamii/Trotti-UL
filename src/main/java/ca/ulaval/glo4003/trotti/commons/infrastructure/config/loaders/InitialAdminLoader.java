...
public class InitialAdminLoader extends Bootstrapper {
    ...
    private void createInitialAdmin() {
        try {
            ...
            Set<Permission> initialAdminPermissions = Set.of(
                    AccountPermissions.CREATE_TECHNICIAN,
                    AccountPermissions.DELETE_TECHNICIAN
            );

            Account admin = accountFactory.createInitialAdmin(
                    name, birthDate, gender, idul, email, password, Role.ADMIN, initialAdminPermissions
            );
