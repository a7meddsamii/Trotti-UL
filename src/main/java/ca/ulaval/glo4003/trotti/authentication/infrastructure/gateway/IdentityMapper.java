...
public class IdentityMapper {
	...
	public Identity toDomain(IdentityAccountDto identityAccountDto) {
		return new Identity(
				identityAccountDto.idul(), Role.fromString(identityAccountDto.role()),
				toDomainPermission(identityAccountDto.permissions()),
				HashedPassword.fromHashed(identityAccountDto.hashedPassword(), identityAccountDto.hasher())
		);
	}
	
	private Set<Permission> toDomainPermission(Set<String> permissionsDto) {
		return permissionsDto.stream()
