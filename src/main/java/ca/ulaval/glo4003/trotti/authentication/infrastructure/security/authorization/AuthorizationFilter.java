...
public class AuthorizationFilter implements ContainerRequestFilter {
	...
	private void validateRole(AnnotatedElement methodCalled, AnnotatedElement resourceClass, SecurityContext securityContext) {
		RolesAllowed allowedRoles = firstNonNullAnnotation(
				methodCalled.getAnnotation(RolesAllowed.class),
				resourceClass.getAnnotation(RolesAllowed.class)
		);
		
		if (allowedRoles != null) {
			boolean ok = Arrays.stream(allowedRoles.value()).anyMatch(securityContext::isUserInRole);
			if (!ok) {
	...
	private void validatePermissions(AnnotatedElement methodCalled, AnnotatedElement resourceClass, UserPrincipal user) {
		RequiresPermissions requiredPermissions = firstNonNullAnnotation(
				methodCalled.getAnnotation(RequiresPermissions.class),
