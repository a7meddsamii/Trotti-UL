package ca.ulaval.glo4003.trotti.trip.infrastructure.config.scheduler;

import ca.ulaval.glo4003.trotti.trip.infrastructure.config.scheduler.jobs.AccountAdvantageRenewalJob;
import ca.ulaval.glo4003.trotti.trip.infrastructure.config.scheduler.jobs.RidePermitActivationJob;
...
public class ServerLifeCycleListener implements ApplicationEventListener {
    ...
    private JobDetail buildJobDetail(Class<? extends Job> jobClass, String jobName,
            String jobGroup) {
        return JobBuilder.newJob(jobClass).withIdentity(jobName, jobGroup).storeDurably().build();
    }

    private Trigger buildTrigger(String cronExpression, String triggerName, String triggerGroup,
            JobKey jobKey) {
        return TriggerBuilder.newTrigger().withIdentity(triggerName + "Trigger", triggerGroup)
                .forJob(jobKey).withSchedule(CronScheduleBuilder.cronSchedule(cronExpression))
                .build();
    }
