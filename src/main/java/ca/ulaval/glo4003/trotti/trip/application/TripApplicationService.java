...
//        }
        ...
        UnlockCode unlockCode = unlockCodeStore.generate(idul, ridePermitId);

        eventBus.publish(new UnlockCodeRequestedEvent(idul,
                ridePermitId.toString(),
                unlockCode.toString(),
                LocalDateTime.now(clock).plusSeconds(60)));
    }

    public void startTrip(StartTripDto tripDto) {
        unlockCodeStore.validate(tripDto.unlockCode(), tripDto.ridePermitId());
...
//      scooterId = scooterRentalGateway.getScooterIdByRidePermitId(tripDto.location(), tripDto.slotNumber());
        ...
        unlockCodeStore.revoke(UnlockCodeKey.from(tripDto.idul(), tripDto.ridePermitId()));
    }
    ...
    public void endTrip(EndTripDto tripDto) {
        ...
        Trip trip = tripRepository.findBy(tripDto.idul(), TripStatus.ONGOING);
