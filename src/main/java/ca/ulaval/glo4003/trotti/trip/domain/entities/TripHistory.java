...
public class TripHistory {
    ...
    public Duration calculateTotalTripsDuration() {
        return completedTrips.stream().map(CompletedTrip::calculateDuration).reduce(Duration.ZERO,
                Duration::plus);
    }
    ...
    public Duration calculateAverageTripDuration() {
        ...
        return numberOfTrips == 0 ? Duration.ZERO
                : calculateTotalTripsDuration().dividedBy(numberOfTrips);
    }

    public Location getFavoriteStartLocation() {
        return completedTrips.stream()
                .collect(Collectors.groupingBy(CompletedTrip::getStartLocation,
                        Collectors.counting()))
    ...
    public Location getFavoriteEndLocation() {
        return completedTrips.stream()
                .collect(
                        Collectors.groupingBy(CompletedTrip::getEndLocation, Collectors.counting()))
                .entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey)
                .orElse(null);
    }
